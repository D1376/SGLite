#pragma once
// Host-side tensor validation and symbolic shape/dtype/device helpers used by the
// TVM FFI entry points before launching low-level kernels.
...
// Device names used in TensorMatcher diagnostics.
    ...
    return stdr::empty(m_options) ||
           (stdr::any_of(m_options, [value](const DLDevice &opt) {
             ...
             if (opt.device_type != value.device_type)
               return false;
             // Device id may be wildcarded by kAnyDeviceID.
...
public:
  ...
  auto with_strides(std::initializer_list<SizeRef> strides) && //
      -> TensorMatcher && {
    // Strides must be specified exactly once and for every dimension.
...
private:
  auto m_verify_impl(tvm::ffi::TensorView view) const -> void {
    ...
    if (this->m_has_strides()) {
      for (const auto i : stdv::iota(std::size_t{0}, dim)) {
        if (view.size(i) != 1 || !m_strides[i]->has_value()) {
          // Size-1 dimensions can have arbitrary strides.
    ...
    // DType and device are always checked, even across chained verify calls.
