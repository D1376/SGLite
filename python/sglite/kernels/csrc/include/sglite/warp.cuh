#pragma once
// Warp-cooperative copy and reset primitives used when one warp is responsible
// for moving one fixed-size embedding or KV-cache record.
...
inline constexpr auto resolve_unit_size(std::size_t x) -> std::size_t {
  ...
  if (x % (4 * kWarpThreads) == 0)
    return 4;
  return 0; // Triggers the static_assert in get_mem_package.
}
