...
// JIT-compiled gather kernels used to pull embedding rows into a dense output
// buffer, optionally zeroing rows that fall outside a masked range.
...
template <std::size_t element_size,        // Bytes per embedding row.
          std::size_t num_splits = 1,      // Warps assigned to one row.
...
struct IndexKernel {
  static void run(const tvm::ffi::TensorView weights,
    ...
    auto D = SymbolicSize{"D"}; // Embedding width.
    auto L = SymbolicSize{"L"}; // Number of indices.
