#include <sglite/tensor.h>
#include <sglite/utils.cuh>
#include <sglite/utils.h>
#include <sglite/warp.cuh>

// JIT-compiled scatter kernel that writes per-token K/V projections into the
// page-based KV-cache layout selected by the scheduler.
...
__global__ __launch_bounds__(kNumThreads, kMaxOccupancy) void //
  ...
  // Each warp writes one K/V element pair.
  if (warp_id < length) {
...
template <std::size_t element_size,        // Bytes per K/V row.
          std::size_t num_threads = 128,   // Threads per block.
...
struct StoreKernel {
  static void run(const tvm::ffi::TensorView k_cache,
    ...
    auto D = SymbolicSize{"D"}; // K/V row width.
    auto L = SymbolicSize{"L"}; // Number of rows.
    auto X = SymbolicSize{"X"}; // KV-cache row stride.
    auto Y = SymbolicSize{"Y"}; // Input row stride.
