#include <sglite/nccl227.h>
#include <sglite/tensor.h>
#include <sglite/utils.cuh>
#include <sglite/utils.h>

// TVM FFI bindings around NCCL collectives plus a reusable symmetric device
// buffer for PyNCCL-backed tensor-parallel communication.
...
public:
  ...
  auto all_reduce(tvm::ffi::TensorView t, std::string op) const -> void {
    ...
    if (size_bytes <= m_max_bytes) {
      // Small reductions use the symmetric internal buffer for NCCL.
  ...
  auto all_gather(tvm::ffi::TensorView dst, tvm::ffi::TensorView src) const
    ...
    // all_gather writes directly into the caller-provided output tensor.
  ...
  TVM_FFI_DECLARE_OBJECT_INFO_FINAL("sglite.NCCLWrapper", NCCLWrapper,
                                    tvm::ffi::Object);
