...
// Small smoke-test export that exercises the tensor matcher utilities against
// representative CPU and CUDA tensor constraints.
...
auto test(tvm::ffi::TensorView x, tvm::ffi::TensorView y) -> void {
  ...
  host::TensorMatcher({N, M})
      .with_strides({-1, 1}) // Any leading stride, contiguous inner dimension.
