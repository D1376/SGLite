...
def _load_nccl_module() -> Module:
    """Load the compiled PyNCCL extension module."""
    return load_aot("pynccl", cuda_files=["pynccl.cu"], extra_ldflags=["-lnccl"])


@functools.cache
def _get_pynccl_wrapper_cls():
    """Register and return the TVM FFI PyNCCL wrapper class."""
    import tvm_ffi

    @tvm_ffi.register_object("sglite.NCCLWrapper")
    class PyNCCLImpl(tvm_ffi.Object):
        def __init__(self, *args):
            """Initialize the FFI-backed PyNCCL object."""
...
def init_pynccl(
    *,
    ...
    """Initialize a PyNCCL communicator for tensor-parallel collectives."""
    import torch
