"""Shared helpers for compiled kernel loading."""
...
class CppArgList(list[str]):
    """Builds C++ argument lists for compiled kernel entry points."""
    def __str__(self) -> str:
        """Render the argument list as a C++ template-argument string."""
        return ", ".join(self)


class KernelConfig(NamedTuple):
    """Compilation configuration for custom kernels."""
    num_threads: int
    max_occupancy: int
    use_pdl: bool
    ...
    def template_args(self) -> str:
        """Render launch configuration as C++ template arguments."""
        pdl = "true" if self.use_pdl else "false"
        return f"{self.num_threads},{self.max_occupancy},{pdl}"


def _make_name(*args: str) -> str:
    """Build a deterministic extension module name from template arguments."""
    return "sglite__" + "_".join(str(arg) for arg in args)


def _make_wrapper(tup: Tuple[str, str]) -> str:
    """Build a TVM FFI export declaration for one kernel wrapper."""
    export_name, kernel_name = tup
    return f"TVM_FFI_DLL_EXPORT_TYPED_FUNC({export_name}, ({kernel_name}));"


def make_cpp_args(*args: CPP_TEMPLATE_TYPE) -> CppArgList:
    """Build a typed C++ template-argument list."""
    def _convert(arg: CPP_TEMPLATE_TYPE) -> str:
        """Convert one Python value into a C++ template literal."""
...
def load_aot(
    *args: str,
    ...
    """Load a prebuilt kernel module from on-disk source files."""
...
def load_jit(
    *args: str,
    ...
    """JIT-compile a kernel module from inline wrapper sources."""
