"""Offline single-process generation interface."""
...
class RequestAllFinished(Exception):
    """Raised when the offline request queue has been drained."""
    pass


@dataclass
class RequestStatus:
    """Tracks prompt and generated tokens for one offline request."""
    uid: int
    input_ids: List[int]
    output_ids: List[int]
...
class LLM(Scheduler):
    """Offline generation wrapper built on top of the scheduler."""
    def __init__(self, model_path: str, dtype: torch.dtype = torch.bfloat16, **kwargs):
        """Create a single-process scheduler for offline generation."""
    ...
    def _tokenize_one(self, prompt: List[int] | str) -> torch.Tensor:
        """Convert one prompt string or token list into CPU int32 token ids."""
    ...
    def offline_receive_msg(self, blocking: bool = False) -> List[BaseBackendMsg]:
        ...
        for tokens_or_prompt, sampling_params in self.pending_requests:
            # Mirror online serving by capping prompt work per prefill step.
    ...
    def offline_send_result(self, reply: List[DetokenizeMsg]) -> None:
        """Collect generated token ids from the offline execution path."""
        for msg in reply:
    ...
    def generate(
        ...
        """Generate output tokens for the supplied prompt or request stream."""
