...
class BackendCreator(Protocol):
    """Callable protocol used by the backend registry."""

    def __call__(self, config: ModelConfig) -> BaseAttnBackend:
        """Build the attention backend for the supplied model configuration."""
        ...
...
def create_trtllm_backend(config: ModelConfig):
    """Create a TensorRT-LLM attention backend."""
...
def create_fi_backend(config: ModelConfig):
    """Create a FlashInfer attention backend."""
...
def create_fa_backend(config: ModelConfig):
    """Create a FlashAttention backend."""
    from .flash_attention import FlashAttentionBackend

    return FlashAttentionBackend(config)


def validate_attn_backend(backend: str, allow_auto: bool = True):
    """Validate an attention backend name or hybrid backend pair."""
    if backend != "auto":
...
def create_attention_backend(
    ...
    """Create the configured attention backend, including hybrid P/D pairs."""
