...
from sglite.srt.utils import is_sm100_supported
...
class FACaptureData(BaseCaptureData):
    """Holds capture buffers for FlashAttention."""
    pass


@dataclass
class FAMetadata(BaseAttnMetadata):
    """Holds metadata for FlashAttention."""
...
class FlashAttentionBackend(BaseAttnBackend):
    """Attention backend implemented with FlashAttention kernels."""
    def __init__(self, config: ModelConfig):
        """Bind KV cache state and select the FlashAttention kernel version."""
        ctx = get_global_ctx()
        self.config = config
        ...
        # TODO: replace this architecture check with an sgl_kernel FA capability probe.
        self.version = 4 if is_sm100_supported() else 3

    def forward(
        self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, layer_id: int, batch: Batch
    ) -> torch.Tensor:
        """Store new KV tensors and run FlashAttention for the layer."""
    ...
    def prepare_metadata(self, batch: Batch) -> None:
        ...
        # Attention kernels expect page ids, while the global table stores token offsets.
    ...
    def prepare_for_replay(self, batch: Batch) -> None:
        ...
        # Decode capture always uses cu_seqlens_q = [0, 1, 2, ..., bs].
...
def _fa_sgl_impl(
    ...
    """Call the SGL kernel FlashAttention wrapper with SGLite metadata."""
    try:
