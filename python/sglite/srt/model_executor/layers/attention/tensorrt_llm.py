"""TensorRT-LLM attention backend implementation."""
...
class TRTLLMCaptureData(BaseCaptureData):
    """Holds capture buffers for TensorRT-LLM."""
    pass


@dataclass
class TRTLLMMetadata(BaseAttnMetadata):
    """Holds metadata for TensorRT-LLM."""
...
class TensorRTLLMBackend(BaseAttnBackend):
    """Attention backend implemented with TensorRT-LLM kernels."""
    def __init__(self, config: ModelConfig):
        """Bind KV cache state and allocate the TensorRT-LLM workspace."""
        ctx = get_global_ctx()
        self.config = config
    ...
    def forward(
        ...
        """Store new KV tensors and run TensorRT-LLM prefill or decode."""
    ...
    def prepare_metadata(self, batch: Batch) -> None:
        ...
        # TensorRT-LLM expects page ids, while the global table stores token offsets.
