"""Attention layer wiring for model blocks."""
...
if TYPE_CHECKING:
    from sglite.srt.model_executor.layers import RMSNorm
    from sglite.srt.model_executor.models import RotaryConfig


class AttentionLayer(StatelessModule):
    """Applies rotary embeddings and dispatches attention backend work."""
    def __init__(
        self,
        layer_id: int,
        num_qo_heads: int,
        ...
        """Compute local head layout and build the RoPE applicator."""
    ...
    def forward(self, qkv: torch.Tensor) -> torch.Tensor:
        """Split QKV, apply optional norms and RoPE, then dispatch attention."""
        ctx = get_global_ctx()
