...
from sglite.srt.utils import div_ceil, nvtx_annotate

from .base import BaseModule


class VocabParallelEmbedding(BaseModule):
    """Tensor-parallel embedding table sharded by vocabulary range."""
    def __init__(
        self,
        num_embeddings: int,
        embedding_dim: int,
    ):
        """Shard the vocabulary range for the current tensor-parallel rank."""
        super().__init__()
        tp_info = get_tp_info()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Gather local embeddings and all-reduce when vocabulary is sharded."""
        from sglite.kernels import indexing
...
class ParallelLMHead(VocabParallelEmbedding):
    """Tensor-parallel projection from hidden states to logits."""
    def __init__(
        self,
        num_embeddings: int,
        embedding_dim: int,
        ...
        """Build an LM head that can reuse the embedding table when tied."""
    ...
    def load_state_dict(
        ...
        *,
        ...
        """Load untied head weights or discard duplicates for tied embeddings."""
        if not self.tied_embedding:
            return super().load_state_dict(state_dict, prefix=prefix, _internal=_internal)
        else:
            # Tied LM heads consume the embedding weights, so discard duplicate head tensors.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Project final hidden states to full-vocabulary logits."""
        ctx = get_global_ctx()
        batch = ctx.batch
        bs = batch.size
        if batch.is_prefill:
