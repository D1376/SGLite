"""Helpers for Marlin quantized kernels."""
...
# Marlin kernel shape constraints.
GPTQ_MARLIN_TILE = 16
...
def get_scalar_types():
    """Return ScalarType helpers from sgl_kernel, or mocks before installation.
...
def get_device_capability() -> tuple:
    """Return the CUDA device capability without forcing CUDA initialization.
...
def check_marlin_supported(
    ...
    """Return whether Marlin supports this quantization configuration.
    ...
    if check_device:
        ...
        # Marlin kernels require Ampere or newer GPUs.
        if device_capability < 80:
            return False

    # Group sizes outside this set do not have generated Marlin kernels.
    if group_size not in MARLIN_SUPPORTED_GROUP_SIZES:
        return False

    # AWQ stores 4-bit weights with zero points; GPTQ-style kernels use different scalar tags.
    if has_zp:
...
def marlin_make_workspace(device: torch.device, max_blocks_per_sm: int = 1) -> torch.Tensor:
    """Create the workspace tensor required by the Marlin kernel.
...
def marlin_make_empty_g_idx(device: torch.device) -> torch.Tensor:
    """Create the empty g_idx tensor required by the Marlin API.
...
def get_scale_perms() -> tuple:
    """Return permutation indices for Marlin scale transformation.
...
def get_pack_factor(num_bits: int) -> int:
    """Return the number of weights packed into each int32 element.
...
def marlin_zero_points(
    ...
    """Transform unpacked zero points into Marlin-packed format.
...
def awq_to_marlin_zero_points(
    ...
    """
    # Expand AWQ's column-packed zero points before applying Marlin's layout.
    q_zp = unpack_cols(q_zp_packed, num_bits, size_k, size_n)

    # Undo AWQ's dequantizer interleave with the inverse permutation.
    if num_bits == 4:
    ...
    # Reapply Marlin's permutation and int32 packing.
...
def apply_awq_marlin_linear(
    ...
    """Apply the AWQ Marlin linear kernel to input activations.
