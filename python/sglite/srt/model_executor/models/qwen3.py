"""Model definitions for the Qwen3 architecture."""
...
from .blocks import GatedMLP as Qwen3MLP
...
class Qwen3DecoderLayer(BaseModule):
    """Decoder block for the Qwen3 architecture."""
    def __init__(self, config: ModelConfig, layer_id: int, linear_method: Optional[LinearMethodBase] = None):
        """Build attention, MLP, and RMSNorm blocks for one Qwen3 layer."""
        self.self_attn = Qwen3Attn(config, layer_id, has_qk_norm=True, linear_method=linear_method)
        self.mlp = Qwen3MLP(config, linear_method=linear_method)
    ...
    def forward(
        ...
        """Run one Qwen3 decoder layer and carry the residual stream forward."""
...
class Qwen3Model(BaseModule):
    """Transformer body for the Qwen3 architecture."""
    def __init__(self, config: ModelConfig, linear_method: Optional[LinearMethodBase] = None):
        ...
        self.layers = ModuleList(
            [Qwen3DecoderLayer(config, layer_id, linear_method=linear_method) for layer_id in range(config.num_layers)]
        )
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        """Embed input ids and run all Qwen3 decoder layers."""
...
class Qwen3ForCausalLM(BaseLLMModel):
    """Causal language model wrapper for the Qwen3 architecture."""
    def __init__(self, config: ModelConfig):
        """Build the Qwen3 transformer body and LM head."""
        linear_method = config.quantization_config.get_linear_method() if config.quantization_config is not None else None
        self.model = Qwen3Model(config, linear_method=linear_method)
