"""Scheduler I/O coordination with worker processes."""
...
import torch
from sglite.srt.messages import BaseBackendMsg, BaseTokenizerMsg, BatchTokenizerMsg, DetokenizeMsg
from sglite.srt.utils import ZmqPubQueue, ZmqPullQueue, ZmqPushQueue, ZmqSubQueue, init_logger
...
class SchedulerIOMixin:
    """Adds tokenizer, detokenizer, and tensor-parallel rank I/O to `Scheduler`."""

    def __init__(self, config: SchedulerConfig, tp_cpu_group: torch.distributed.ProcessGroup):
        """Wire scheduler IPC queues according to rank and offline mode."""
    ...
    def run_when_idle(self):
        """Perform scheduler-defined background work while blocking for input."""
        raise NotImplementedError("should be implemented")

    def offline_receive_msg(self, blocking: bool = False) -> List[BaseBackendMsg]:
        """Receive one message from the offline execution path."""
        raise NotImplementedError("should be implemented")

    def offline_send_result(self, reply: List[DetokenizeMsg]) -> None:
        """Send one result through the offline execution path."""
        raise NotImplementedError("should be implemented")
    ...
    def _recv_msg_single_rank(self, blocking: bool = False) -> List[BaseBackendMsg]:
        """Receive pending tokenizer messages in a single-rank deployment."""
    ...
    def _recv_msg_multi_rank0(self, blocking: bool = False) -> List[BaseBackendMsg]:
        """Receive tokenizer messages on rank 0 and broadcast them to followers."""
        pending_msgs: List[BaseBackendMsg] = []
        if blocking:
            self.run_when_idle()
        ...
        # Tell follower ranks how many raw messages they must read from rank 0.
    ...
    def _recv_msg_multi_rank1(self, blocking: bool = False) -> List[BaseBackendMsg]:
        """Receive rank-0 broadcasts on follower scheduler ranks."""
        pending_msgs: List[BaseBackendMsg] = []
        if blocking:
            self.run_when_idle()
            pending_msgs.append(self._recv_from_rank0.get())

        # Rank 0 broadcasts the exact message count before publishing the payloads.
    ...
    def _reply_tokenizer_rank0(self, reply: List[DetokenizeMsg]) -> None:
        """Send generated-token replies from rank 0 to the detokenizer."""
        num_reply = len(reply)
    ...
    def _reply_tokenizer_rank1(self, reply: List[DetokenizeMsg]) -> None:
        """Ignore replies on follower ranks because rank 0 owns output I/O."""
        _ = reply
