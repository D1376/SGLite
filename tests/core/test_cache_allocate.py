...
class TestAllocateEvictPageAlignment:
    ...
    def test_lazy_free_region_does_not_rebuild_free_slots_when_empty(self):
        """An empty lazy-free region should not allocate a replacement free-list tensor."""
        cm = _make_cache_manager(num_pages=4, page_size=4)
        data_ptr = cm.free_slots.data_ptr()

        with cm.lazy_free_region():
            pass

        assert cm.free_slots.data_ptr() == data_ptr
